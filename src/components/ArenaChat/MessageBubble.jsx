    ...
    <div className={`mb-3 d-flex ${isSender ? 'justify-content-end' : 'justify-content-start'}`}>
      <div
        style={{
          ...
          backgroundColor: isSender ? '#222' : '#003300', // Gray for user, green for received
      ...
      >
        <div style={{
          ...
          {!isSender && <strong className="sender-name">{message.sender??"You"}</strong>}
        </div>
        <p className="mb-1" style={{ marginBottom: '0.5rem' }}>{message?.content??message?.message?.content}</p>
        <div style={{
